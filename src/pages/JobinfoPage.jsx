...
const JobinfoPage = () => {
  ...
  useEffect(() => {
    fetch("/data/jobsInfopage.json") // If the file is in the `public/` folder
  ...
  return (
    <>
      <main className="flex flex-col bg-purple-100">
        ...
        <div className="flex flex-col space-y-2 mx-2 lg:flex-row lg:mx-auto mt-2 justify-start lg:space-x-3 lg:items-start xl:mt-4 xl:mx-[5rem] 2xl:mx-[32rem] xl:justify-between 2xl:justify-start">
          {" "}
          {/* job info columns */}
          <JobInfoCard
        ...
        <div className="flex flex-col my-2">
          ...
          <div className="flex flex-row justify-between mx-2 lg:mx-20 xl:mx-35 mt-2 mb-2 2xl:mx-[32rem]">
            ...
            <button className="text-xs md:text-md text-purple-800 font-bold bg-white shadow-lg rounded p-2" onClick={() => navigate("/jobs")}>
              View more
            </button>
          </div>
          <div className="container mx-2 xl:mx-auto grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 xl:gap-8 justify-center space-x-1">
          {jobs.map((job, index) => (
                          <Jobcard
                            key={index}
                            companylogo={job.companyLogo}
        ...
        <div className="bg-white mt-5"> {/* Footer */}
        <Footer />
        </div>
      </main>
    </>
